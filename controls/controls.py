...
def _find_defaut_font():
    ...
    #print (fonts)
    df = {}
    checkeds = ["arial","helvetica","segoeui","calibri","tahoma","verdana"]
    for f in fonts:
        #print(f)
        for t in checkeds:
            if t == f:
...
class Control:
    ...
    KEY_CHECK_DISABLED  : Final[str]= "check_disabled"
    ...
    MODES = {
        DAY_MODE:{
            ...
            KEY_BK              :(255,255,255),
            KEY_CHECK           :(0,0,0),
            KEY_CHECK_DISABLED  :(127,127,127),
        ...
        GREEN_MODE:{ 
            KEY_HINT            :(255,255,127),
            KEY_TEXT            :(255,255, 0),
            KEY_SELECTED        :(255,127, 255),
            KEY_INACTIVE_TXT    :(192, 192, 205),
            KEY_BK              :(0,127,0),
            KEY_CHECK           :(255,255,255),
            KEY_CHECK_DISABLED  :(192,192,192),
        ...
        GRAY_MODE:{ 
            KEY_HINT            :(255,255,127),
            KEY_TEXT            :(255,255, 255),
            KEY_SELECTED        :(255,255, 0),
            KEY_INACTIVE_TXT    :(255, 212, 212),
            KEY_BK              :(127,127,127),
            KEY_CHECK           :(255,255,255),
            KEY_CHECK_DISABLED  :(127,127,127),
            KEY_SELECTED_ITEM   :(127,0,0),
            KEY_AROW            :(192,192,192),  
            KEY_CBO_LINE        :(127,127,127),
            KEY_BTN             :(220, 220, 220),
            KEY_BTN_CHNG        :(255, 0, 0) ,
            KEY_ON              :(  0,127,0),
    ...
    CLR_CHECK_DISABLED  = MODES[mode][KEY_CHECK_DISABLED]
    ...
    def __init__(self,name,controlType,font_name = None,font_size= None,font_italic=None,
                 func = None,text = '',clr_text = None,hint_clr=None):
       # self.value: float = 0.0
        self.name = text if name == '' else name
        self.hide = False
        self.type = controlType
        self.func = func
        if font_name != None or font_size != None or font_italic != None:
            ...
            italic            = False             if font_italic == None else font_italic
            self.font         = pygame.font.SysFont(self.font_name, self.font_size,Control.FONT_BOLD,italic=italic) 
            if text != None:
    ...
    def set_hint(self,hint):
        if self.hint != None and hint != None:
            if self.hide == hint:
                return
        self.hint = hint
...
def set_contols_mode(mode):
    ...
    Control.CLR_CHECK_DISABLED  = mode_values[Control.KEY_CHECK_DISABLED]
