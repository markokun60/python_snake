...
class Form:
    ...
    def check_cbo(self):
        f = True
        for c in self.controls.values():
            if not c.hide and c.type == Control.COMBOBOX and c.enabled:
                if c.expanded:
                    f = False
                    break
    ...
    def _activate_from_list(self,list):
        ...
        cFirst = None
        for c in list:
            if not c.hide and c.type != Control.LABEL and c.enabled: 
                if cFirst == None:
                    cFirst = c
                if c.active:
                    b = 1
    ...
    def handle_controls_events(self,event):
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_TAB:
                ...
                for c in self.controls.values():
                    if not c.hide and c.type == Control.BUTTON and c.enabled: 
                        if c.key != None:
                            if event.key == c.key:
                                c.call_back()
                                return True
