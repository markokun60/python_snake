...
class Checkbox(Control):
    ...
    def __init__(self, name,position: Tuple[float, float] = (0, 0),text: str='' ,checked: bool = False,func=None,style= STYLE_BOX):
        ...
        self.txr_surf_disabled = self.font_selected.render(self.text, 1, Control.CLR_INACTIVE_TXT)   
    ...
    def draw(self, surface: pygame.Surface):
        if self.style == Checkbox.STYLE_BAR:
            x = self.rect_check.left
            y = self.rect_check.top
            clr_check = Control.CLR_CHECK if self.enabled else Control.CLR_CHECK_DISABLED
            if self.checked:
                clr = Control.CLR_ON if self.enabled else Control.CLR_CHECK_DISABLED
                pygame.draw.rect(surface,clr,(x,y,self.rect_check.width - Checkbox.CHECK_SIZE,self.rect_check.height))
                pygame.draw.rect(surface,clr_check,self.rect_check,1) 
                y += (self.rect_check.height - self.off.get_height())// 2  
                surface.blit(self.on,(x,y))
            else:    
                pygame.draw.rect(surface,Control.CLR_INACTIVE_TXT,
                                (x + Checkbox.CHECK_SIZE ,y,self.rect_check.width - Checkbox.CHECK_SIZE,self.rect_check.height))
                pygame.draw.rect(surface,clr_check,self.rect_check,1)
                y += (self.rect_check.height - self.off.get_height())// 2 
                x +=  self.rect_check.width   - self.off.get_width() - 4
            ...
            clr = Control.CLR_CHECK if self.enabled else Control.CLR_CHECK_DISABLED
            checkmark = []
            #print(self.rect)
            ...
            pygame.draw.rect(surface, clr ,self.rect_check, 1)
            if self.checked:
                pygame.draw.lines(surface, clr, False, checkmark, Checkbox.BORDER)
        ...
        if self.active:
            ...
            surface.blit(self.txr_surf_disabled ,(x,y))
